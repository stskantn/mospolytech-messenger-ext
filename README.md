...
- Файл сохраняется в том же формате, что был загружен (TXT → TXT, CSV → CSV)
- Автоматическое именование: `original_name_with_ids.txt`
...
1. **Текст Юникод (TXT)** - поддержка файлов, сохраненных в MS Office 2013 как "Текст Юникод"
2. **CSV UTF-8** - по-прежнему поддерживается

### Как использовать:
#### Вариант 1: Текст Юникод (TXT) - для MS Office 2013
...
#### Вариант 2: CSV UTF-8 (для новых версий Excel)
1. Создайте таблицу в Excel с двумя столбцами
2. Нажмите `Файл` → `Сохранить как`
